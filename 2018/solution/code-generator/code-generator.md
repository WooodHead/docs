# open source code generator 

## Java


### code running based on model description file

    - [entando/component-generator](https://github.com/entando/component-generator )

### code running based on database sql

    - [shalousun/ApplicationPower](https://github.com/shalousun/ApplicationPower )
    - [jeeffy/code-generator](https://github.com/jeeffy/code-generator)
    - [Java代码生成器](https://github.com/zhaohaihao/CodeGenerator )
    - [xxl-code-generator](https://github.com/xuxueli/xxl-code-generator )
    - [firebata/Code-Generator](https://github.com/firebata/Code-Generator )

### maven-plugin

    - [jaxio/celerio](https://github.com/jaxio/celerio )
    - [nutzwk-code-generator](https://github.com/enilu/nutzwk-code-generator )
    - [baomidou/mybatisplus-maven-plugin](https://github.com/baomidou/mybatisplus-maven-plugin )
    
### gradle-plugin
   
    - [kingcos/MyBatisGenerator-Tool](https://github.com/kingcos/MyBatisGenerator-Tool )
   
### intellij-plugin

    - [hykes/CodeGen](https://github.com/hykes/CodeGen )
    - [heyuxian/code-generator](https://github.com/heyuxian/code-generator )
    
### swing 

    - [NutzCodematic](https://github.com/Wizzercn/NutzCodematic )
    - [biezhi/swing-generate](https://github.com/biezhi/swing-generate )
    
### umlgen

    - [eclipse umlgen](https://www.eclipse.org/umlgen/ )
   
### tools for generate jpa ,mybits, java file ,or others

   - [astav/JsonToJava](https://github.com/astav/JsonToJava )
   - [mkarneim/pojobuilder](https://github.com/mkarneim/pojobuilder )
   - [Pyknic/CodeGen](https://github.com/Pyknic/CodeGen )
   - [smartnews/jpa-entity-generator](https://github.com/smartnews/jpa-entity-generator )
   - [goudai/gd-generator](https://github.com/goudai/gd-generator )
   - [domaframework/doma-gen](https://github.com/domaframework/doma-gen )
   - [AmirHooshangi/DTO-code-generator](https://github.com/AmirHooshangi/DTO-code-generator )
   - [stateforge/StateBuilder](https://github.com/stateforge/StateBuilder )
   - [vcodecc/mybatis-generator-utils](https://github.com/vcodecc/mybatis-generator-utils )
   
## typescript
  
   - [graphql code generator](https://github.com/dotansimha/graphql-code-generator )  
  
## python
   - [swagger py-codegen](https://github.com/guokr/swagger-py-codegen )
   
## c++
   - [libclang](http://clang.llvm.org/docs/Tooling.html#libclang )
   - [Implementing a code generator with libclang](http://szelei.me/code-generator/ )
   
## Android

   - [AlbieLiang/ArbitraryGen](https://github.com/AlbieLiang/ArbitraryGen )
   
## api
   - [swagger](https://github.com/swagger-api/swagger-codegen )
  